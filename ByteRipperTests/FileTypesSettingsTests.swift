...
final class FileTypesSettingsTests: XCTestCase {
    ...
    override func setUp() {
        ...
        selfHandled = []
        handlerNames = ["bin": "Archive Utility", "rom": "ByteRipper"]
        handlerIDs = ["bin": "com.apple.archiveutility", "rom": "dev.maxik.ByteRipper"]
        selfHandled = ["rom"]
        setSelfCalls = []
        setHandlerCalls = []
        messages = []
    ...
    func testTheTableShowsTheListAndReadsTheSystem() throws {
        ...
        XCTAssertEqual(tab.handlerLabel(atRow: 1), "ByteRipper")
    }
    ...
    func testTheTabRereadsTheSystemWhenItAppears() throws {
        ...
        handlerNames["bin"] = "ByteRipper"
        tab.viewWillAppear()

        XCTAssertEqual(tab.checkbox(atRow: 0)?.state, .on)
        XCTAssertEqual(tab.handlerLabel(atRow: 0), "ByteRipper")
    }
    ...
    func testTickingAsksTheSystemAndRemembersWhoHeldTheType() throws {
        ...
        tab.setSelfAsDefault = { [unowned self] ext, then in
            ...
            self.handlerNames[ext] = "ByteRipper"
            then(nil)
        }
