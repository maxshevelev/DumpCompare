...
final class FavoritesFileTests: XCTestCase {
    ...
    // MARK: - The folder the app was renamed out of

    /// The rename from DumpCompare to ByteRipper kept the bundle identifier, so
    /// an updating user's library is in the same container under the old folder
    /// name. It is moved, once, the first time the app looks for it.
    func testTheOldFoldersLibraryIsMovedToTheNewName() throws {
        let base = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let legacy = base.appendingPathComponent("DumpCompare", isDirectory: true)
        try FileManager.default.createDirectory(at: legacy, withIntermediateDirectories: true)
        try Data("kept".utf8).write(to: legacy.appendingPathComponent("Favorites.json"))
        defer { try? FileManager.default.removeItem(at: base) }

        let resolved = FavoritesFile.directory(in: base)

        XCTAssertEqual(resolved.lastPathComponent, "ByteRipper")
        XCTAssertEqual(try Data(contentsOf: resolved.appendingPathComponent("Favorites.json")),
                       Data("kept".utf8),
                       "the patterns came with the folder")
        XCTAssertFalse(FileManager.default.fileExists(atPath: legacy.path),
                       "and the old folder is gone, so the move cannot run twice")
    }

    /// A folder under the new name already there is the one in use, whatever is
    /// beside it — a second run must not overwrite the library with an older one.
    func testAnExistingNewFolderIsLeftAlone() throws {
        let base = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let legacy = base.appendingPathComponent("DumpCompare", isDirectory: true)
        ...
        try Data("old".utf8).write(to: legacy.appendingPathComponent("Favorites.json"))
        try Data("current".utf8).write(to: current.appendingPathComponent("Favorites.json"))
        defer { try? FileManager.default.removeItem(at: base) }
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: legacy.path),
                      "the old folder is not ours to delete")
    }

    /// A fresh install has neither folder, and gets the new name.
    func testAFreshInstallGetsTheNewFolder() {
        let base = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(UUID().uuidString, isDirectory: true)

        XCTAssertEqual(FavoritesFile.directory(in: base).lastPathComponent, "ByteRipper")
    }
}
