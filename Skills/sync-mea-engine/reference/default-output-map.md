...
| 4 | Type | always | `fw_type` — Stock / Update / Extracted classifier at 12538–12574 (IFWI ⇒ "Extracted" + FIT fields from `bpdt_hdr`; else `$FPT`-partition/FOVD/`FitBuild` heuristics) | [P] **`type` is a different axis** — engine hardcodes `.region`; enum already has `extracted`/`update` but the classifier is unwired (see §5 gap list) |
...
| 6b | Chipset | same gate & `sku_stp == 'Unknown'` | literal `"Unknown"` | [E] fallback logic (UI) |
| 6c | Chipset Stepping | same gate & otherwise | `", ".join(sku_stp)` — stepping letters from DB (`sku_stp`, 10275–10280) or `pch_init_final[-1][1]` fallback (10308) | [P] `chipsetStepping` top-level is PMC-only today; main-table stepping for a no-`pch_init` case unwired |
| 7 | NVM Compatibility | `nvm_db` truthy | `ext15_info[3]` — CSE_Ext_0F_R2 `NVMCompatibility` (SPI/UFS, 6246–6248 via `ext15_nvm_type`) | [A] byte value (Int) not surfaced; label map `ext15_nvm_type` is UI text |
...
| 12a | Power Down Mitigation | `[variant,major]==['CSME',11]` & `pdm_status != 'NaN'` | `pdm_status` | [F] no CSME-11 dump; add fixture-only |
| 12b | Workstation Support | `[variant,major]==['CSME',11]` | `['No','Yes'][fw_0C_lbg]` (0x0C capability) | [F] no CSME-11 dump; add fixture-only |
| 13 | Patsburg Support | (ME, 7) | `['No','Yes'][is_patsburg]` | [F] no ME-7 dump; add fixture-only |
| 14 | OEM Configuration | variant in `CSME/CSTXE/CSSPS/TXE/GSC` | `['No','Yes'][int(oem_signed or oemp_found or utok_found)]` — OEM RSA-signed (`oem_signed`, 6016), OEM-partition (`oemp_found`, 11790/12133), UTOK/STKN present (`utok_found`, 11786/12129) | [P] `oemConfiguration` = FITC *region* facts; upstream's Bool is a different OEM-signed/presence test — verify mapping |
| 15 | FWUpdate Support | (CSME & major≥12) | `fwu_iup_result` — `'No'/'Yes'/'Impossible'/'Unknown'` from the IUP FWUpdate-presence check (13601–13611) | [D] needs the whole-flash independent (PMCP/PCHC/PHY) scan (§5) |
| 16 | Date | always | `date` — manifest date `"%0.4X-%0.2X-%0.2X"` (5918) | [E] `manufactureDate` (from manifest day/month/year) |
| 17 | File System State | variant in `CSME/CSTXE/CSSPS/GSC` | `mfs_state` — `'Initialized'/'Configured'/'Unconfigured'/'Error'` from the MFS scan (`mfs_parsed_idx` 7489–7493; init 11075; raised by EFS/config presence 13050–13051 etc.) | [A] MFS decode exists (`mfsVolume`) but no state enum is derived (§5) |
| 18 | Size | `rgn_exist` or `cse_lt_struct` or variant starts `PMC/PCHC/PHY/OROM`; (ME,6,'ROM-Bypass') ⇒ `"Unknown"`; (CSTXE, fd_devexp) ⇒ skip | `"0x%X" % eng_fw_end` — end of the analyzed firmware inside the flash (11792–11793; per-family module-end 12292–12486; OROM 13511; IUP 13538–13577) | [P] `sizeBytes` = the **whole analyzed region length** (e.g. 0x1000000), not MEA's `eng_fw_end` (0x27C000) — see §5 |
| 19 | Flash Image Tool | `fitc_ver_found` | `get_fw_ver(variant, fitc_major, fitc_minor, fitc_hotfix, fitc_build)` — FIT version from the BPDT header `bpdt_hdr.FitMajor/…` (12542–12545) | [A] BPDT header is decoded (`bootPartitions`) but the FIT `major.minor.hotfix.build` is not surfaced |
| 20 | Manifest Extension Utility | `mn2_meu_ver != '0.0.0.0000'` | `mn2_meu_ver` — FTPR manifest MEU block fields (12229) | [A] manifest MEU not surfaced (see §5) |
| 21 | Downgrade Blacklist 7.0 / 7.1 | (ME, 7) | `me7_blist_1` / `me7_blist_2` | [F] no ME-7 dump; add fixture-only |
| 22 | Chipset Support | `platform != 'NaN'` | `platform` — per-family PCH/Southbridge text (legacy literal tables 12748–12913, e.g. `ICH8`/`IBX`) | [X] DB/display text → parked (result-model rule); absent on the real CSME-12 case (see §3) |
...
bumps `EngineModelRevision`, currently **21** — see `result-model.md`).

| Status | Row(s) | Detail |
|---|---|---|
| **EXISTS** | Family, Version, Release (partial), SKU, TCB SVN, Date | Engine `family`+`variant`, `version.text`, `release` (Engineering suffix rule unwired), `sku` (value matches), `securityVersion`, `manufactureDate`. |
| **PARTIAL — mapping/formatting** | Chipset (6a), Chipset Stepping (6c), OEM Configuration, Size (18) | 6a: `mfsVolume?.pchInit?.chipsets` present but row shows only the last record as `"chipset " + steppings joined ","`. 6c: top-level `chipsetStepping` is PMC-only. OEM Config: `oemConfiguration` = FITC *region*, upstream Bool = OEM-signed/presence test — verify. Size: engine `sizeBytes` = analysed-region length ≠ MEA `eng_fw_end`; renderer needs the firmware-end layout fact. |
| **ADD — byte-core fact** | ARB SVN (9), VCN (10), Production Ready (11), File System State (17), Type/fw_type (4), Flash Image Tool (19), Manifest Extension Utility (20) | CSE_Ext_0F `ARBSVN`; CSE_Ext_03/0F `VCN` (top-level, distinct from R0-only `ManifestSummary.vcn`); FTPR header pv bit; an MFS-state enum derived from the existing `mfsVolume` decode; the Stock/Update/Extracted classifier (model `type` is hardcoded `.region` today); BPDT-header FIT version; FTPR MEU version. Each is byte-decoded already upstream and needs a Swift field + revision bump. |
| **ADD — fixture-only later** | Power Down Mitigation, Workstation Support (12a/12b), Patsburg Support (13), Downgrade Blacklist (21), NVM Compatibility (7) | CSME-11 / ME-7 / modern-NVM cases: no real-dump oracle in this project; code + unit fixtures only, `swift test` as the gate. |
...
| **PARKED — DB/UI display text** | Chipset Support (22); the three export-only DB rows | `platform` text and `name_db`/support-status/RSA-hash rows are DB-derived display text, excluded by the result-model rule (`databaseName`/`signatureHash` exist in the model but never hit the console). |
