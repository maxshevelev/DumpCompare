...
bumps `EngineModelRevision`, currently **24** — see `result-model.md`).

| Status | Row(s) | Detail |
|---|---|---|
| **EXISTS** | Family, Version, Release (partial), SKU, TCB SVN, Date, Type (4), OEM Configuration (14), Flash Image Tool (19) | Engine `family`+`variant`, `version.text`, `release` (Engineering suffix rule unwired), `sku` (value matches), `securityVersion`, `manufactureDate`. Row 4 `type` = `FirmwareTypeClassifier` (gating per §2; ME 2–7's unported `.unknown` legs and unidentified images keep the row grey). Row 14 `oemCustomized: Bool?`. Row 19 = boot-`BPDT.fit*` on IFWI ("N/A" when FIT-less), model `fptHeaderFIT` on the non-IFWI real-FIT Extracted branch only. |
| **PARTIAL — mapping/formatting** | Chipset (6a), Chipset Stepping (6c), Size (18) | 6a: `mfsVolume?.pchInit?.chipsets` present but row shows only the last record as `"chipset " + steppings joined ","`. 6c: top-level `chipsetStepping` is PMC-only. Size: engine `sizeBytes` = analysed-region length ≠ MEA `eng_fw_end`; renderer needs the firmware-end layout fact. |
| **ADD — byte-core fact** | ARB SVN (9), VCN (10), Production Ready (11), File System State (17), Manifest Extension Utility (20) | CSE_Ext_0F `ARBSVN`; CSE_Ext_03/0F `VCN` (top-level, distinct from R0-only `ManifestSummary.vcn`); FTPR header pv bit; an MFS-state enum derived from the existing `mfsVolume` decode; FTPR MEU version. Each is byte-decoded already upstream and needs a Swift field + revision bump. |
