...
| **ADD — fixture-only later** | Patsburg Support (13), Downgrade Blacklist (21) | ME-7 rows: no ME-7 dump in the oracle set, so code + unit fixtures only, `swift test` as the gate. (The CSME-11 rows 12a/12b are done — `old.bin` turned out to be a CSME 11.8 oracle for both.) |
