...
| 6c | Chipset Stepping | same gate & otherwise | `", ".join(sku_stp)` — stepping letters from DB (`sku_stp`, 10275–10280) or `pch_init_final[-1][1]` fallback (10308) | [P] `chipsetStepping` top-level is PMC-only; the summary picks one chipset row (6a → 6c → promise) under the CS/PMC/GSC gate, and an identified image with neither fact shows the row grey rather than guessing "Unknown" — the DB `sku_stp` lookup is unported |
...
bumps `EngineModelRevision`, currently **25** — see `result-model.md`).

| Status | Row(s) | Detail |
|---|---|---|
| **EXISTS** | Family, Version, Release (partial), SKU, NVM Compatibility (7), TCB SVN, Date, Type (4), OEM Configuration (14), Flash Image Tool (19), Manifest Extension Utility (20) | Engine `family`+`variant`, `version.text`, `release` (Engineering suffix rule unwired), `sku` (value matches), `securityVersion`, `manufactureDate`. Row 4 `type` = `FirmwareTypeClassifier` (gating per §2; ME 2–7's unported `.unknown` legs and unidentified images keep the row grey). Row 14 `oemCustomized: Bool?`. Row 19 = boot-`BPDT.fit*` on IFWI ("N/A" when FIT-less), model `fptHeaderFIT` on the non-IFWI real-FIT Extracted branch only. Row 7 `nvmCompatibility` (raw bits + UI label map), row 20 `version.me*` with upstream's marker gate. |
...
| **ADD — byte-core fact** | — | ARB SVN (9), VCN (10), Production Ready (11), File System State (17) and Manifest Extension Utility (20) are all surfaced now (`arbSvn`, `vcn`, `ManifestSummary.productionReady`, `mfsState`, `version.me*`). What is left of this class is the `eng_fw_end` layout fact behind row 18, listed under PARTIAL. |
| **ADD — fixture-only later** | Power Down Mitigation, Workstation Support (12a/12b), Patsburg Support (13), Downgrade Blacklist (21) | CSME-11 / ME-7 cases: no real-dump oracle in this project; code + unit fixtures only, `swift test` as the gate. |
