...
| `MFS_Config_Record_*`, `MFS_Home_Record_*`, `MFS_Integrity_Table_*`, `MFS_Backup_Header_R0/R1`, `MFS_Backup_Entry` | MFS low-level *file* walk: FAT `record → data-FAT slot → chunk` chains assembled into each file's raw content (`MFSLowLevelFile.index/content`), EOF marker 1…0x40 gives the final chunk's used bytes; non-empty records surfaced as `MFSFile` (index→size) on `MFSVolume` + `presentFileCount`/`fileBytes`, a corrupt/cyclic chain clears `fileChainsIntact` and raises Issue id 13. Byte-verified on both dumps (CSME 12: 210 present files, total 0x12464, record 6 Intel Config = 0x4A97; CSME 15: 136). Home/config/integrity/backup *record* decode over that content (`mfs_home13_anl`/`mfs_cfg_anl`, rows 71/96) still deferred | `FileSystem/MFS.swift` | partial |
| `UTFL_Header`, `FITC_Header` | misc CSE tables | `FileSystem/Misc.swift` | deferred — same |
| (deferral note) | The raw `MFS` region exists on **both** real dumps (CSME 12 & 15) and its page-inventory + low-level *file walk* are decoded (rows 66–67); the newer EFST/EFS/FTBL/UTFL/FITC tables and the MFS home/config *record* decode sit inside the Huffman-compressed `vfs`/`fpf`/module bodies (or need `FileTable.dat` naming) — decoding them needs the decompression size targets (Phase 8) plus `FileTable.dat` naming (row 93), so those stay open | — | deferred |
...
| `cse_part_inid`, `ext_anl`, `mod_anl`, `mfs_anl`, `mfs_home_anl`, `mfs_cfg_anl`, `efs_anl`, `fitc_anl`, `mfs_home13_anl`, `get_sec_hdr_size`, `get_cfg_rec_size`, `get_vfs_start_0`, `get_mfs_anl` | walking/decode drivers — the *structural portions* already split out: `ext_anl`/`mod_anl` row facts (70), `mfs_anl` volume decode + low-level *file* walk (72, 67), CSE-LT/BPDT (40–41); the home/config *record* decode (`mfs_home13_anl`, `mfs_cfg_anl`) and the `efs`/`fitc` walkers sit inside the compressed module bodies + need `FileTable.dat` naming, so they follow rows 63–69 | `FileSystem/*.swift` | deferred — record decode (rows 66–67/96) |
