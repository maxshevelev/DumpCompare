...
| `cse_part_inid`, `ext_anl`, `mod_anl`, `mfs_anl`, `mfs_home_anl`, `mfs_cfg_anl`, `efs_anl`, `fitc_anl`, `mfs_home13_anl`, `get_sec_hdr_size`, `get_cfg_rec_size`, `get_vfs_start_0`, `get_mfs_anl` | walking/decode drivers — the *structural portions* already split out: `ext_anl`/`mod_anl` row facts (70), `mfs_anl` volume decode + low-level *file* walk (72, 67), the legacy (non-FTBL) half of `mfs_cfg_anl` (0x1C record decode, 67), the legacy (non-FTBL) home-directory + Integrity decode (`mfs_home_anl`, `get_sec_hdr_size`, `get_vfs_start_0` — `MFSHomeDecoder`, row 67), the structural half of `efs_anl`/`fitc_anl` (`EFSParser`/`FITCParser`, rows 65/68), CSE-LT/BPDT (40–41); the FTBL-based `mfs_home13_anl` naming, the FTBL 0xC branch of `mfs_cfg_anl`, the EFS file-*content* walk and the FITC config-*record* walk (all `FileTable.dat` naming), and the MFS backup *record* decode remain open and follow rows 63–69/93 | `FileSystem/*.swift` | partial — home/integrity + EFS/FITC structural done; FTBL content naming / records + backup decode (rows 65/67/68/96) open |
