...
| `cse_part_inid`, `ext_anl`, `mod_anl`, `mfs_anl`, `mfs_home_anl`, `mfs_cfg_anl`, `efs_anl`, `fitc_anl`, `mfs_home13_anl`, `get_sec_hdr_size`, `get_cfg_rec_size`, `get_vfs_start_0`, `get_mfs_anl` | walking/decode drivers — the *structural portions* already split out: `ext_anl`/`mod_anl` row facts (70), `mfs_anl` volume decode + low-level *file* walk (72, 67), the legacy (non-FTBL) half of `mfs_cfg_anl` (0x1C record decode, 67), CSE-LT/BPDT (40–41); the home-directory record decode (`mfs_home_anl`/`mfs_home13_anl`), the FTBL 0xC branch of `mfs_cfg_anl` (needs `FileTable.dat` naming), the Integrity/backup record decode and the `efs`/`fitc` walkers sit inside the compressed module bodies + need `get_sec_hdr_size`/`get_cfg_rec_size` variant selection and `FileTable.dat`, so they follow rows 63–69 | `FileSystem/*.swift` | deferred — home/FTBL/EFS/FITC record decode (rows 67/96) |
