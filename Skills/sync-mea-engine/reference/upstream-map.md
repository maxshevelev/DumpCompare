...
| (deferral note) | The raw `MFS` region exists on **both** real dumps (CSME 12 & 15) and its page-inventory + low-level *file walk* are decoded (rows 66–67), the legacy (non-FTBL) Intel/OEM Configuration 0x1C *record* decode is done (row 67), and the legacy (non-FTBL) file-8 Home Directory + per-file `MFS_Integrity_Table` decode is done (row 67, `MFSHomeDecoder`); the newer EFST/EFS/FTBL/UTFL/FITC tables and the FTBL-based home naming (`mfs_home13_anl`) / FTBL-0xC config *record* decode sit inside the Huffman-compressed `vfs`/`fpf`/module bodies (or need `FileTable.dat` naming) — decoding them needs the decompression size targets (Phase 8) plus `FileTable.dat` naming (row 93), so those stay open | — | deferred |
