...
| `cse_part_inid`, `ext_anl`, `mod_anl`, `mfs_anl`, `mfs_home_anl`, `mfs_cfg_anl`, `efs_anl`, `fitc_anl`, `mfs_home13_anl`, `get_sec_hdr_size`, `get_cfg_rec_size`, `get_vfs_start_0`, `get_mfs_anl` | walking/decode drivers — the *structural portions* already split out: `ext_anl`/`mod_anl` row facts (70), `mfs_anl` volume decode + low-level *file* walk (72, 67), the legacy (non-FTBL) half of `mfs_cfg_anl` (0x1C record decode, 67), the legacy (non-FTBL) home-directory + Integrity decode (`mfs_home_anl`, `get_sec_hdr_size`, `get_vfs_start_0` — `MFSHomeDecoder`, row 67), CSE-LT/BPDT (40–41); the FTBL-based `mfs_home13_anl` naming, the FTBL 0xC branch of `mfs_cfg_anl` (both need `FileTable.dat` naming), the MFS backup *record* decode and the `efs`/`fitc` walkers sit inside the compressed module bodies / need `FileTable.dat`, so they follow rows 63–69/93 | `FileSystem/*.swift` | partial — home/integrity done; FTBL/EFS/FITC/backup record decode (rows 67/96) open |
