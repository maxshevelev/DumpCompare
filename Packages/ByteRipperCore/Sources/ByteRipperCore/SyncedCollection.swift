...
/// One thing a synced collection holds (`Design/FAVORITES_SYNC_PLAN.md`).
///
/// The favourites were the first of these, and nothing about carrying them
/// between two Macs is about *patterns*: an item needs an identity of its own,
/// a place in the order, when it last changed and which machine changed it, and
/// the merge does the rest. Anything else the app may want to keep on every Mac
/// — bookmarks, segments — is the same problem with different content, and this
/// is where the answer lives rather than in a copy of it.
public protocol SyncedItem: Equatable, Sendable, Codable {
    /// The item's own identity, minted once and kept for its lifetime. What
    /// makes a rename a rename rather than a deletion plus an addition.
    var id: UUID { get }
    /// Where it sits in the list: a number between its neighbours', not an
    /// index — two machines renumbering the same list have nothing left to
    /// merge.
    var sortKey: Double { get set }
    /// When it last changed, by the clock of the machine that changed it. A
    /// tiebreak, never the arbiter of a merge.
    ...
    /// refuses to hold it twice — two names for one search (§11). Collections
    /// that hold no such rule inherit the default and never say yes.
    func isDuplicate(of other: Self) -> Bool
}

public extension SyncedItem {
    func isDuplicate(of other: Self) -> Bool { false }
}
...
public struct SyncTombstone: Equatable, Sendable, Codable {
    public let id: UUID
...
/// A collection two machines keep in step: what is written to a file, and what
/// they exchange (`Design/FAVORITES_SYNC_PLAN.md`).
///
/// JSON rather than a plist, because the argument for a file at all includes
/// being able to read a diff of it — a collection under version control, or
/// sent to someone, has to be legible.
///
/// Beside the items, everything the syncing needs: the deletions, the count of
/// writes per machine, and which Mac wrote this copy. A format that gains
/// fields later is a format with two readers, and the file is the one thing
/// another machine's build has to understand.
/// The constants the syncing works to.
///
/// Here rather than on `SyncedCollection` because a generic type cannot hold a
/// stored static — and here rather than scattered because every one of them is
/// a promise between two machines' builds.
public enum Sync {
    /// The format a file is written in. Read back defensively: a file from a
    /// newer build is not thrown away, it is read for what this build knows.
    public static let currentFormat = 1

    /// The gap left between neighbours in the order. Large enough that
    /// inserting between two of them repeatedly halves for a long time before
    ...
    /// grow forever. Getting it wrong resurrects an item.
    ...
    /// every "is this the same collection" comparison quietly false.
...
public struct SyncedCollection<Item: SyncedItem>: Equatable, Sendable {
    ...
    public static var currentFormat: Int { Sync.currentFormat }

    public init(entries: [Item] = [], tombstones: [SyncTombstone] = [],
                vector: VersionVector = VersionVector(), machine: String = "",
                format: Int = Sync.currentFormat) {
        self.format = format
    ...
    /// A deleted item: which one, when, and by whom (`SyncTombstone`).
    public typealias Tombstone = SyncTombstone
    ...
    public mutating func setOrder(_ list: [Item]) {
    ...
    /// The gap left between neighbours (`Sync.sortKeyStep`).
    public static var sortKeyStep: Double { Sync.sortKeyStep }
...
extension SyncedCollection: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        // by a build that had no tombstones yet is a valid collection, not an
        // error to report to someone who only wants their patterns back.
        format = try container.decodeIfPresent(Int.self, forKey: .format) ?? Sync.currentFormat
        entries = try container.decodeIfPresent([Item].self, forKey: .entries) ?? []
        tombstones = try container.decodeIfPresent([SyncTombstone].self, forKey: .tombstones) ?? []
...
extension SyncedCollection {
    ...
    static func encoder() -> JSONEncoder {
        ...
        encoder.dateEncodingStrategy = .custom { date, encoder in
            ...
            try container.encode(Sync.timestamps.string(from: date))
        }
        return encoder
    }
    ...
    static func decoder() -> JSONDecoder {
        ...
        decoder.dateDecodingStrategy = .custom { decoder in
            ...
            guard let date = Sync.timestamps.date(from: text)
                    ?? Sync.wholeSeconds.date(from: text) else {
