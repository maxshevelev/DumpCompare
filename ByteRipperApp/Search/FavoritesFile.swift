...
enum FavoritesFile {
    ...
    static func defaultURL() -> URL {
        ...
        return directory(in: base).appendingPathComponent("Favorites.json")
    }
    ...
    /// `Application Support/DumpCompare`. The bundle identifier deliberately did
    /// *not* change with the name, so this is the same container and the old
    /// folder is simply sitting beside a new one that does not exist yet.
    ///
