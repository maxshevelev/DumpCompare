...
final class FolderSync<Kind: SyncedCollectionKind> {
    ...
    func start() {
        ...
        // NSLog("ByteRipper library: starting, rules of %@, device %@, writing %@",
    ...
    func resolve(_ answers: [UUID: SyncResolution]) {
        // NSLog("ByteRipper library: answering %d question(s) with %@",
        //       conflicts.count, String(describing: answers.values.map { "\($0)" }))
        guard !conflicts.isEmpty else {
            // NSLog("ByteRipper library: nothing to answer — the questions had already gone")
            return
        }
        ...
        // NSLog("ByteRipper library: after answering — %d question(s), local=%@ %@, error=%@",
    ...
    func sync() {
        ...
        // if let publishError {
        //     NSLog("ByteRipper: could not publish the pattern library to %@ — %@",
    ...
    private func absorb(_ url: URL, raising raised: inout [SyncConflict<Item>],
        ...
        guard outcome.isResolved else {
            ...
            // NSLog("ByteRipper library: %d question(s) from %@ — base=%@ %@ ours=%@ %@ theirs=%@ %@",
